...
$lineMax = 155; // maxiumum pixels per line
array_shift($argv);
...
while(count($argv)){
	...
	if(get_text_width($lineText . ' ' . $word) <= $lineMax){
		...
		while(get_text_width($lineText) < $lineMax){
			$lineText .= ' ';
		}
...
echo scrubForDynamic($lineText) . "[BREAK]\n";

function scrubForDynamic($string){
	return str_replace(
		array("\n", "\\n", ','),
		array('[BREAK]', '[BREAK]', '[COMMA]'),
		$string
	);
}
