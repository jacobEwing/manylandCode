function change(creation){
	...
	var radius = ((width > height ? height : width) >> 1);
	var center = radius;
	...
	sourcePoints = [
		...
		{x : (3 * center) >> 1, y : 0},
		{x : width - 1, y : 0},
		{x : width - 1, y : center >> 1},
		{x : width - 1, y : center},
		{x : width - 1, y : (3 * center) >> 1},
		{x : width - 1, y : height -1},
		{x : (3 * center) >> 1, y : height - 1},
		{x : center, y : height - 1},
		{x : center >> 1, y : height - 1},
		{x : 0, y: height - 1},
		{x : 0, y: (3 * center) >> 1},
		{x : 0, y: center},
		{x : 0, y: center >> 1},
	...
	for(var ang = 0; ang <= 2 * Math.PI; ang += Math.PI / 8){
		targetPoints[targetPoints.length] = {
			x : Math.round(Math.sin(ang) * radius),
			y : - Math.round(Math.cos(ang) * radius)
		};
	}
/*
